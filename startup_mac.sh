...
################################################################################
# Realtime Pipeline Kafka-Pinot - Complete Startup Script (macOS)
# This script starts all services required for the application on macOS
################################################################################
...
preflight_checks() {
    print_header "Pre-flight Checks (macOS)"
...
install_python_requirements() {
    ...
    # Use python3 -m pip for better compatibility on macOS
    if python3 -m pip install -r requirements.txt; then
        print_success "Core dependencies installed successfully"
    else
        print_error "Failed to install core dependencies"
        print_info "You may need to run: python3 -m pip install --upgrade pip"
        exit 1
    fi
...
start_docker_services() {
    ...
    if command -v gtimeout &> /dev/null; then
        if gtimeout 10 curl -s http://localhost:9000/health > /dev/null 2>&1; then
...
setup_kafka_topics() {
    ...
    # List topics
    TOPIC_COUNT=$(docker exec kafka kafka-topics --list --bootstrap-server localhost:9092 2>/dev/null | grep -v "^_" | wc -l | tr -d ' ')
...
initialize_database() {
    ...
    # Verify database (macOS tr may behave differently)
...
verify_system() {
    ...
    # Check Kafka topics (macOS wc adds spaces)
...
display_info() {
    ...
    echo -e "     ${YELLOW}Note: Install watch with: brew install watch${NC}"
    echo ""
    ...
    echo -e "\n${BLUE}macOS Specific Notes:${NC}"
    echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
    echo -e "  • If you see performance issues, allocate more resources to Docker Desktop"
    echo -e "  • Docker Desktop → Preferences → Resources → Advanced"
    echo -e "  • Recommended: 4GB+ RAM, 2+ CPUs"
    echo -e "  • Install Homebrew packages: ${YELLOW}brew install coreutils watch${NC}"
    echo ""
...
main() {
    ...
    echo -e "${BLUE}"
    echo "╔════════════════════════════════════════════════════════════════╗"
    echo "║     Realtime Pipeline Kafka-Pinot - macOS Startup             ║"
    echo "╚════════════════════════════════════════════════════════════════╝"
    echo -e "${NC}\n"
